...
class EnhancedDashboardDetector:
    def __init__(self, config_file=None):
        ...
        self.dashboard_height = 300
        self.dashboard_bg_color = (40, 40, 40)  # Dark gray
        self.alert_color = (0, 0, 255)  # Red
        self.safe_color = (0, 255, 0)   # Green
    ...
    def load_config(self, config_file):
        ...
        default_config = {
            ...
            'output_video_name': 'drowsiness_detection_output.avi',
            'fps': 15,
            'alarm_stop_delay': 3.0,
            'min_open_frames': 10,
            'dashboard_enabled': True,
            'dashboard_position': 'right',  # right, left, top, bottom
            'show_ear_graph': True,
    ...
    def eye_aspect_ratio(self, eye_landmarks):
        ...
        try:
            # Compute euclidean distances between landmarks
    ...
    def get_landmarks(self, image):
        ...
        try:
            ...
            scaled_rect = dlib.rectangle(
                int(largest_face.left() * self.config['face_downsample_ratio']),
                int(largest_face.top() * self.config['face_downsample_ratio']),
                int(largest_face.right() * self.config['face_downsample_ratio']),
                int(largest_face.bottom() * self.config['face_downsample_ratio'])
            )
    ...
    def check_eye_status(self, landmarks):
        ...
        try:
            ...
            eye_status = 1 if avg_ear > threshold else 0
            
            return eye_status, avg_ear, threshold
    ...
    def calculate_alertness_score(self):
        ...
        try:
            ...
            # Factor 1: Recent drowsy episodes (last 30 minutes)
            if self.drowsy_episodes_today > 0:
                score -= min(self.drowsy_episodes_today * 15, 50)
            
            # Factor 2: Average EAR (higher is better)
            ...
            # Factor 3: Blink rate (normal is 15-20 per minute)
    ...
    def draw_dashboard(self, frame):
        ...
        try:
            ...
            if self.config['dashboard_position'] == 'right':
                # Create extended frame for right dashboard
            ...
            # Dashboard title
            cv2.putText(extended_frame, "DRIVER MONITOR", 
                       (dashboard_x + 10, dashboard_y + 25), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.text_color, 2)
            
            # Current time
            current_time = datetime.now().strftime("%H:%M:%S")
            cv2.putText(extended_frame, current_time, 
            ...
            score_color = self.safe_color if alertness_score > 80 else \
                         self.warning_color if alertness_score > 60 else self.alert_color
            
            cv2.putText(extended_frame, f"Alertness: {alertness_score:.0f}%", 
                       (dashboard_x + 10, dashboard_y + 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, score_color, 2)
            
            # Draw alertness bar
            bar_width = 200
            bar_height = 10
            bar_x = dashboard_x + 10
            bar_y = dashboard_y + 70
            
            # Background bar
            cv2.rectangle(extended_frame, (bar_x, bar_y), (bar_x + bar_width, bar_y + bar_height),
                         (100, 100, 100), -1)
            
            # Alertness bar
            filled_width = int((alertness_score / 100) * bar_width)
            cv2.rectangle(extended_frame, (bar_x, bar_y), (bar_x + filled_width, bar_y + bar_height),
                         score_color, -1)
            
            # Statistics
            y_offset = dashboard_y + 100
            stats_to_show = [
                f"Blinks: {self.stats['total_blinks']}",
                f"Drowsy Episodes: {self.drowsy_episodes_today}",
            ...
            for i, stat in enumerate(stats_to_show):
                cv2.putText(extended_frame, stat, 
                           (dashboard_x + 10, y_offset + i * 25), 
    ...
    def draw_ear_graph(self, frame, x, y):
        ...
        try:
            ...
            cv2.rectangle(frame, (x, y), (x + graph_width, y + graph_height), self.text_color, 1)
            
            # Graph title
            cv2.putText(frame, "EAR History", (x + 5, y - 5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, self.text_color, 1)
            
            # Draw EAR line
            if len(self.ear_history) > 1:
                ear_data = list(self.ear_history)[-60:]  # Last 60 values
                max_ear = max(ear_data) if ear_data else 0.5
                min_ear = min(ear_data) if ear_data else 0.1
                
                # Normalize data
                if max_ear > min_ear:
                    normalized_data = [(val - min_ear) / (max_ear - min_ear) for val in ear_data]
                else:
                    normalized_data = [0.5] * len(ear_data)
                ...
                for i in range(1, len(normalized_data)):
                    ...
                    cv2.line(frame, (x1, y1), (x2, y2), self.safe_color, 2)
                
                # Draw threshold line
                threshold = self.config['ear_threshold']
                if min_ear <= threshold <= max_ear:
                    threshold_y = y + graph_height - int(((threshold - min_ear) / (max_ear - min_ear)) * graph_height)
                    cv2.line(frame, (x, threshold_y), (x + graph_width, threshold_y), self.alert_color, 1)
    ...
    def draw_status_indicators(self, frame, x, y):
        ...
        try:
            # Status indicators
            indicators = [
                ("CAMERA", self.safe_color if self.capture and self.capture.isOpened() else self.alert_color),
                ("SOUND", self.safe_color if self.config['sound_enabled'] else (100, 100, 100)),
                ("RECORDING", self.safe_color if self.vid_writer else (100, 100, 100)),
                ("ALARM", self.alert_color if self.alarm_on else (100, 100, 100))
            ]
    ...
    def sound_alert(self, sound_path):
        ...
        try:
            ...
            if platform.system() == "Windows":
                import winsound
                while True:
                    if not self.thread_status_q.empty():
                        if self.thread_status_q.get():
                            break
                    try:
    ...
    def calibrate(self):
        ...
        while valid_frames < calibration_frames:
            try:
                ...
                frame = cv2.resize(frame, None, 
                                 fx=1/image_resize, fy=1/image_resize, 
                                 interpolation=cv2.INTER_LINEAR)
                ...
                if landmarks is None or confidence < self.config['confidence_threshold']:
                    cv2.putText(frame, "Please position your face properly", 
                              (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    cv2.putText(frame, f"Calibration: {valid_frames}/{calibration_frames}", 
        ...
        print(f"SPF: {self.spf*1000:.2f} ms")
        print(f"Drowsy limit: {self.drowsy_limit:.1f} frames")
        print(f"False blink limit: {self.false_blink_limit:.1f} frames")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if landmarks is None or confidence < self.config['confidence_threshold']:
                        cv2.putText(frame, "Face not detected - Check lighting", 
                        ...
                        if self.alarm_on and self.eyes_open_time is not None:
                            ...
                            if remaining_time > 0:
                                cv2.putText(frame, f"Alarm stops in: {remaining_time:.1f}s", 
                                          (50, frame.shape[0] - 100), cv2.FONT_HERSHEY_SIMPLEX, 
                                          0.8, (0, 255, 255), 2)
...
def main():
    ...
    parser.add_argument("--dashboard-position", choices=['right', 'left', 'top', 'bottom'], 
                       help="Dashboard position")
